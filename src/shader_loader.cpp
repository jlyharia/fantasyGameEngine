...
char *shader_loader::textFileRead(char *fn) {
...
int shader_loader::textFileWrite(char *fn, char *s) {
...
int shader_loader::printOglError(char *file, int line) {
    //
...
void shader_loader::printShaderInfoLog(GLuint obj) {
    int infologLength = 0;
    int charsWritten = 0;
    char *infoLog;
...
void shader_loader::printProgramInfoLog(GLuint obj) {
    int infologLength = 0;
    int charsWritten = 0;
    char *infoLog;
...
GLuint shader_loader::createShaders(std::string file_name) {
...
GLuint shader_loader::createShaders(std::string vs_shader, std::string fs_shader) {

}

std::string shader_loader::find_shader(std::string file_name, ShaderType shaderType) {
    GLuint shader = glCreateShader(shaderType.getShaderType());

    std::string vert_shader = file_name + ".vs.glsl";
    for (auto prefix: file_prefix) {
        for (auto ext :shaderType.getExtension()) {
            std::string filePath = prefix + file_name + ext;
            if (is_file_exist(filePath)) {
                std::cout << filePath << "\n";
            }
        }
    }
//    fileExist(vert_shader);
//    char *vs = textFileRead((char *) vert_shader.c_str());
}